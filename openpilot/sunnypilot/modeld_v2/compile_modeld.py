...
if __name__ == "__main__":
  ...
  if is_unified_supercombo:
    ...
    for cam_w, cam_h in set(args.camera_resolutions):
      ...
      p = ctx.Process(target=_compile_unified_resolution_worker, args=(
        cam_w, cam_h, args.supercombo_onnx, model_w, model_h, derived_frame_skip, args.benchmark_runs, tmp_res.name))
      p.start()
      p.join()
      if p.exitcode != 0:
        raise RuntimeError(f"Unified compilation worker failed for {cam_w}x{cam_h}")
    ...
    for cam_w, cam_h, tmp_name in tmp_files:
      ...
      output_data['input_devices']['model'] = dev_name
      os.remove(tmp_name)
      gc.collect()
  else:
